...
class PlateDetector:
    def load_model(self, weight_path: str, cfg_path: str):
        ...
        with open("weights/detection/skripsi.names", "r") as f:
    ...
    def draw_labels(self, boxes, confidences, class_ids, img):
        ...
        for i in range(len(boxes)):
            if i in indexes:
                ...
                gry = cv2.cvtColor(crop_img, cv2.COLOR_BGR2GRAY)
                # thr = cv2.threshold(gry, 220, 255, cv2.THRESH_BINARY_INV + cv2.THRESH_OTSU)[1]

                try:
                    crop_resized = cv2.resize(gry, dsize=(470, 110))
                    plats.append(crop_resized)

                    cv2.rectangle(img, (x, y), (x + w, y + h), color_green, 1)
                    confidence = round(confidences[i], 3) * 100
                    cv2.putText(img, str(confidence) + "%", (x + 20, y - 20), font, 0.5, (0, 255, 0), 1)
